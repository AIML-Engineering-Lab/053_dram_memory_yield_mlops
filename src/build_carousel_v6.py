"""Build carousel v6 HTML:
Removed slides: architecture GIF, GPU comparison
Removed diagrams: 5-layer MLOps arch diagram, deployment stack on day40
Monitoring dashboard: 20% larger
Fixed: all images properly embedded (no formatter stripping)
Fixed: blank page at end removed
...
for k in imgs:
    if imgs[k].startswith(PREFIX):
        imgs[k] = imgs[k][len(PREFIX):]

# Quick sanity check
for k, v in imgs.items():
    assert len(v) > 1000, f"Image '{k}' looks too short: {len(v)} chars"
    assert not v.startswith("data:"), f"Image '{k}' still has URI prefix after strip"
...
.num-g{color:var(--green)}.num-o{color:var(--orange)}.num-r{color:var(--red)}
...
def slide(num, total, content, is_last=False):
    extra_cls = " last" if is_last else ""
...
s3 = f"""  <div class="tag tag-cyan" style="margin-bottom:18px">A100 GPU Training</div>
  ...
  <div class="caption" style="margin-top:0">
    <p><span class="key">Why AUC-PR, not AUC-ROC?</span> At 1:160 imbalance, AUC-ROC inflates from massive true negatives. We catch 21.6% of defects but AUC-ROC shows 0.816. AUC-PR ignores true negatives and honestly reflects minority class detection. Our 0.054 is 8.6x better than random (0.00625).</p>
  </div>"""
...
s4 = f"""  <div class="tag tag-red" style="margin-bottom:20px">Critical Engineering Insight</div>
  ...
  <div class="caption" style="margin-top:0">
    <p><span class="key">Why loss goes high, stays, then drops:</span> Early epochs: model is random (high loss). Middle: learns easy majority first; FocalLoss then forces focus on hard minority. Late: cosine LR schedule stabilizes convergence.</p>
  </div>"""

# ── SLIDE 5: 40-Day Drift Heatmap + PSI ──
s5 = f"""  <div class="tag tag-cyan" style="margin-bottom:16px">40-Day PSI Drift Heatmap</div>
...
# ── SLIDE 8: Monitoring Dashboard ONLY (no architecture diagram, 20% bigger) ──
s8 = f"""  <div class="tag tag-cyan" style="margin-bottom:14px">Production Monitoring</div>
  <h2 style="margin-bottom:14px;font-size:42px">Real-time drift +<br><span class="ac">latency tracking in Grafana</span></h2>
  <div class="img-box img-box-contain" style="height:720px;margin-bottom:14px">
    <img src="data:image/png;base64,{imgs['monitoring']}" alt="Grafana monitoring dashboard">
  </div>
  <div class="caption" style="margin-top:0">
    <p><span class="key">Grafana shows:</span> Request rate, latency percentiles (p50/p95/p99), prediction distribution (pass/fail ratio), CPU usage with HPA threshold, error rate, and pod autoscaling. All scraped by Prometheus every 15 seconds from the FastAPI <code style="color:var(--cyan);font-family:monospace">/metrics</code> endpoint. Alerts fire when p99 &gt; 500ms or error rate &gt; 1%.</p>
  </div>"""

# ── SLIDE 9: Hardware Benchmark ──
s9 = f"""  <div class="tag tag-cyan" style="margin-bottom:16px">Hardware Benchmark</div>
...
# ── SLIDE 10: 40-Day Production Timeline ──
s10 = f"""  <div class="tag tag-orange" style="margin-bottom:10px">40-Day Production Run</div>
  ...
  <div class="caption">
    <p><span class="key">How to read this:</span> 40 production days map to 40 real production days (Feb 20 to Mar 31, 2026). Each day processes 5M rows of DRAM sensor data. The system autonomously handled stable operation, gradual drift, a false alarm, triggered retraining, caught a bad model via canary testing, and auto-rolled back. Zero human intervention at any point.</p>
  </div>"""

# ── SLIDE 11: Day 40 (no deployment stack, no extra images) ──
s11 = f"""  <div class="tag tag-green" style="margin-bottom:10px">Final State: Day 40</div>
  <h2 style="margin-bottom:10px;font-size:38px">System recovered.<br><span class="ag">Self-healing pipeline proven.</span></h2>
  <div class="img-box img-box-contain" style="height:750px;margin-bottom:14px">
    <img src="data:image/png;base64,{imgs['day40']}" alt="Day 40 - 3 panel drift visualization">
  </div>
  <div class="caption" style="margin-top:0">
    <p><span class="key">3-panel drift visualization (Day 40 of 40).</span> Top: train vs production distribution for retention_time_ms showing clear shift. Middle: PSI per day with severity markers, retrain (Day 30) and rollback (Day 39) events. Bottom: AUC-PR showing v1 degradation, v2 recovery, canary failure, and final recovery. Full 40-frame animated GIF: <code style="color:var(--cyan);font-family:monospace">assets/drift_density_animation.gif</code></p>
  </div>"""
...
<head>
<meta charset="UTF-8">
<title>DRAM Memory Yield Predictor - MLOps Carousel v6</title>
<style>
{CSS}
</style>
</head>
...
{slide(9, TOTAL, s9)}
{slide(10, TOTAL, s10)}
{slide(11, TOTAL, s11, is_last=True)}
</body>
</html>"""
...
print(f"Carousel v6 written: {out} ({size_kb}KB)")
print(f"Slides: {TOTAL}")

# Verify all images are properly embedded
import re
imgs_in_html = re.findall(r'<img\s[^>]*>', html, re.DOTALL)
print(f"\nImage check ({len(imgs_in_html)} total):")
for img in imgs_in_html:
    alt = re.search(r'alt="([^"]*)"', img)
    src = re.search(r'src="data:image/png;base64,([A-Za-z0-9+/]{20})', img)
    alt_t = alt.group(1) if alt else "?"
    status = "OK" if src else "BROKEN"
    print(f"  {status}  {alt_t}")
