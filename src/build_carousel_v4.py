"""Build carousel v4 HTML with all fixes:
- No em dashes anywhere
- Timeline spans full page width
- Day 40 static GIF image on final page
- Tool screenshots/descriptions for Airflow, MLflow, Grafana, Prometheus, FastAPI
- Architecture + Monitoring dashboard images
- Concept explanations (FocalLoss, HybridTransformerCNN, AUC-PR, PSI, features)
- 8.5GB parquet note, wall clock explanation
...
.ar{color:var(--red)}.ay{color:var(--yellow)}
...
TOTAL = 11  # expanded from 9 to 11 slides
...
s2 = f"""  <div class="tag tag-red" style="margin-bottom:14px">The Problem</div>
  ...
  <table class="feat-tbl">
    ...
    <tr><td>gate_oxide_thickness_a</td><td>Oxide layer (&angst;ngstroms)</td><td>Too thin = breakdown</td></tr>
...
s3 = f"""  <div class="tag tag-cyan" style="margin-bottom:18px">A100 GPU Training</div>
  ...
  <div class="caption" style="margin-top:0">
    <p><span class="key">Why AUC-PR, not AUC-ROC?</span> At 1:160 imbalance, AUC-ROC inflates from massive true negatives. We catch 21.6% of defects but AUC-ROC shows 0.816. AUC-PR ignores true negatives and honestly reflects minority class detection. Our 0.054 is 8.6x better than random (0.00625). <span class="key">A100-SXM4-40GB</span> is the same GPU on Colab Pro, server-grade NVLink socket with 40GB HBM2e memory.</p>
  </div>"""
...
# ── SLIDE 8: Architecture + Monitoring ──
s8 = f"""  <div class="tag tag-cyan" style="margin-bottom:14px">System Architecture</div>
...
# ── SLIDE 10: 40-Day Simulation Timeline ──
s10 = f"""  <div class="tag tag-orange" style="margin-bottom:10px">40-Day Production Run</div>
  ...
  <div class="img-box img-box-contain" style="height:380px;margin-bottom:10px">
    <img src="data:image/png;base64,{imgs['sim_summary']}" alt="Simulation summary">
  </div>
  <div class="stats-row">
  ...
  <div class="caption">
    <p><span class="key">How to read this:</span> 40 simulation days map to 40 real production days (Feb 20 to Mar 31, 2026). Each day processes 5M rows of DRAM sensor data. "Wall clock" = 219 minutes of actual GPU compute time to simulate the entire 40-day run. The system autonomously handled stable operation, gradual drift, a false alarm, triggered retraining, caught a bad model via canary testing, and auto-rolled back. Zero human intervention at any point.</p>
  </div>"""

# ── SLIDE 11: Day 40 Image + GIF reference + Deployment ──
...
<head>
<meta charset="UTF-8">
<title>DRAM Memory Yield Predictor - MLOps Carousel v4</title>
<style>
{CSS}
</style>
</head>
...
out.write_text(html)
print(f"Carousel v4 written: {out} ({len(html)//1024}KB)")
